...
import com.example.liuzheng.myapplication.model.SonModel;
...
public class SecondActivity extends Activity implements OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getBumenNames(MainActivity.getInstance().jsonUtils.parentModelList);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (parentModels != null && parentModels.size() != 0) {
            //获取下一级name
            List<String> data = getNames(parentModels.get(position).getId(), MainActivity.getInstance().jsonUtils.parentModelList
                    , MainActivity.getInstance().jsonUtils.sonModelList,position);
